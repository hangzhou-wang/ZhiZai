	...
	$(function(){
		...
		if ($.trim(textContent).length!=0) {
			...
			$.ajax({
				...
				dataType:'json',
				error:function(){
					$('.panel-body').append('<div class="talkr"><div class="wordr"><div class="commentRobot">连接出错</div></div><img class="headPhotoRobot" title="头像" alt="头像" align="absmiddle" src="<%=basePath %>static/images/robot.png"/></div>');
				},
				success:function(data){
					if ($.trim(data.code).length==0) {
						$('.panel-body').append('<div class="talkr"><div class="wordr"><div class="commentRobot">Sorry!!系统繁忙……</div></div><img class="headPhotoRobot" title="头像" alt="头像" align="absmiddle" src="<%=basePath %>static/images/robot.png"/></div>');
					}else{
...
<body style="background:url(<%=basePath %>static/images/robotBkg.png) no-repeat;" onkeypress="go()">
	<a href="<%=basePath %>goto/toLearn.do">
		<button id="teach" type="button" class="btn btn-primary">教Jarvis说话</button>
	</a>
	<div id="test" style="width:300px;height:300px;overflow-y:scroll;">
