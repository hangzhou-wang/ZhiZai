	...
	$(function(){
		$("#goBtn").click(function(){
			...
			if ($.trim(textContent).length!=0) {
				$('.panel-body').append('<div class="talkp"><div class="wordp"><div class="comment">'+textContent+'</div></div><img class="headPhoto" title="头像" alt="头像" align="absmiddle" src="static/images/headPic.png"/></div>');
				$('#textSc').val('');
				$.ajax({
					url:'<%=basePath %>/robot/talk.do',
					type:'post',
					data:{"yourWord":textContent},
					dataType:'json',
					error:function(){
						$('.panel-body').append('<div class="talkr"><div class="wordr"><div class="commentRobot">连接出错</div></div><img class="headPhotoRobot" title="头像" alt="头像" align="absmiddle" src="static/images/robot.png"/></div>');
					},
					success:function(data){
						if ($.trim(data.code).length==0) {
							$('.panel-body').append('<div class="talkr"><div class="wordr"><div class="commentRobot">Sorry!!系统繁忙……</div></div><img class="headPhotoRobot" title="头像" alt="头像" align="absmiddle" src="static/images/robot.png"/></div>');
						}else{
							$('.panel-body').append('<div class="talkr"><div class="wordr"><div class="commentRobot">'+data.word+'</div></div><img class="headPhotoRobot" title="头像" alt="头像" align="absmiddle" src="static/images/robot.png"/></div>');
						}
					}
					
				});
			}else{
				$('.panel-body').append('<div class="talkp"><div class="wordp"><div class="comment">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</div></div><img class="headPhoto" title="头像" alt="头像" align="absmiddle" src="static/images/headPic.png"/></div>');
				$('#textSc').val('');
				$.ajax({
					url:'<%=basePath %>/robot/talk.do',
					type:'post',
...
<body style="background:url(static/images/robotBkg.png) no-repeat;" onkeypress="go()">
